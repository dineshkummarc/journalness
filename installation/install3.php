...
$filePerms	= getParam( $_POST, 'filePerms', '');
$dirPerms	= getParam( $_POST, 'dirPerms', '');
$configArray['siteUrl'] = trim( getParam( $_POST, 'siteUrl', '' ) );
$configArray['absolutePath'] = trim( getParam( $_POST, 'absolutePath', '' ) );
				...
				while($file2 = readdir($handle2)){
					if ($file2 == '.' || $file2 == '..'){
						...
						if(is_dir($completepath2)){
							...
							$display = ucfirst($language);
							$options .= "<option value=\"$language\">$display</option>";
	...
	<div class="install">
		...
		<div id="right">
			...
			<div class="install-text">
				  <p>If the URL and Path look correct then please do not change them.
				  ...
				  <br/>
				  The permission settings will be used while installing Journalness itself and
				  by the Journalness extras-installers. If you are unsure
				  what flags shall be set, leave the default settings.</p>
			</div>
...
?>						<td align="center"><input class="inputbox" type="text" name="absolutePath" value="<?php echo $abspath; ?>" size="50"/></td>
					...
					<tr>
						...
						<td align="left"><select name="def_language"><?php echo getLanguages(); ?></select></td>
					</tr>
					<tr>
						<td>User Activation</td>
						<td align="left"><input type="radio" name="user_activation" value="1" label="Yes" />Yes <input type="radio" name="user_activation" value="0" checked="checked" />No</td>
					</tr>
					<tr>
<?php
	$mode = 0;
